...
const BYTE	Cfg_Descp_Mass[] = {0x09,				// bLength: Size of descriptor
                         ...
                         39,0x00,		// wTotalLength: Cfg+Ifc+Class+Ep = 32 bytes
...
static void updatenandsectors(unsigned long sector, U32 sectorcnt)
{
	...
	for(blocknum=startblock; blocknum<endblock+1; blocknum++)
	{
		...
		phy_block = nand_get_phy_block(blocknum);
		phy_block_buff = phy_block % nand_planecnt;
#endif
		
#ifdef COPY_BACK
		if(nand_phy_eraseblock(phy_block_buff) == FALSE)
		{
...
#ifdef COPY_BACK
		U32 zeropagenum;		
		zeropagenum = (phy_block_buff*pageperblock);
#else
		...
		{
#ifdef COPY_BACK
			for(pagenum=blocknum*pageperblock, phy_page = nand_get_phy_page(pagenum);
				pagenum<blocknum*pageperblock+pageperblock;
				pagenum++, phy_page++, zeropagenum++)
			{
				...
				else
				{
					...
					nand_phy_copyback(phy_page, zeropagenum);
				}
			}
#else
			for(pagenum=blocknum*pageperblock;
				pagenum<blocknum*pageperblock+pageperblock;
				pagenum++)
			{
				if((pagenum >= startsector) && (pagenum <= endsector))
...
#ifdef COPY_BACK
		for(pagenum=0, phy_page = nand_get_phy_page((blocknum*pageperblock)+pagenum);
			pagenum<pageperblock;
			pagenum++, phy_page++)
		{
			nand_phy_copyback((phy_block_buff*pageperblock)+pagenum, phy_page);
		}
...
#ifdef SPIFLASH_STORAGE
void updatespisectors(unsigned long sector,U32 sectorcnt)
{	
	ALIGN4 U8 tempbuf[4*1024];
	
	unsigned int sectorperblock = (4*1024)/512; // 8
...
static void WriteData(unsigned long baddr,U32 blkcnt)
{
	...
	for(k=0;k<blkcnt;k++)
	{
		...
		SDC_Write_Sect(buf,baddr+k,1);
	}
	
#elif defined NAND_STORAGE
	updatenandsectors(baddr,blkcnt);
	
#elif defined SPIFLASH_STORAGE
	updatespisectors(baddr,blkcnt);
...
static void SendReadData(unsigned long baddr,U32 blkcnt)
{
	...
	if(FALSE == spi_readpage_block_wusb(baddr,blkcnt) )
	{
		debugprintf("spiflash_readpage error\r\n");
	}
...
#elif defined SPIFLASH_STORAGE
	SPIFLASH_init();
	fatpagecnt = (SPI_GetSectorCount())-1;
	// PRINTVAR((SPI_GetSectorCount()));
	// PRINTVAR(fatpagecnt);
...
void mass_storage_main()
{
	...
	debugstring("\r\n================================================\r\n");	
	debugprintf("Press any key for Return Menu\r\n");
	debugstring("================================================\r\n");	
	
	while(1)
	{    
		if(UartGetCh(0, &key)!=0) 
		{		
			*R_PMCTRLEN |= PMCTRL_SCLK_GATE_EN;
			*R_SCLKGATE &= ~(SCLK_GATE_USB12_EN | SCLK_GATE_USB48_EN); //USB Clock Disable		
			*R_PMCTRLEN = 0;		
			*R_P6oLOW = (1<<4);//PIO6.4 high , off	
			break;
		}
