...
int i2c_read(unsigned char SLAVE_ADDRESS, unsigned short REG_ADDRESS, unsigned char *read_buffer, int read_byte)
{
    ...
    if((write(i2c_fd, reg_addr, 2)) < 0){
        printf("REG_ADDRESS Write Fail\r\n");
        return -1;
    }

    if((read(i2c_fd, read_buffer, read_byte)) < 0){
        printf("I2C(PCA9665) Read Fail\r\n");
        return -1;
    }    
    return 0;
}
...
int i2c_write(unsigned char SLAVE_ADDRESS, unsigned short REG_ADDRESS, unsigned char *write_buffer, int write_byte)
{
    ...
    tx_buffer = (unsigned char *)malloc((sizeof(unsigned char) * write_byte) + 2);
    ...
    if((write(i2c_fd, tx_buffer, write_byte + 2)) < 0){
...
int i2c_write_test(unsigned char SLAVE_ADDRESS, unsigned short REG_ADDRESS, unsigned char *write_buffer, int write_byte)
{
    ...
    tx_buffer = (unsigned char *)malloc((sizeof(unsigned char) * write_byte) + 1);
    ioctl(i2c_fd, I2C_SLAVE_FORCE, SLAVE_ADDRESS);
    tx_buffer[0] = REG_ADDRESS & 0x00FF;
    memmove(&tx_buffer[1], write_buffer, write_byte);

    if((write(i2c_fd, tx_buffer, write_byte + 1)) < 0){
        printf("I2C(PCA9665) Write Fail\r\n");
        free(tx_buffer);
        return -1;
    }
...
int i2c_read_test(unsigned char SLAVE_ADDRESS, unsigned short REG_ADDRESS, unsigned char *read_buffer, int read_byte)
{
    ...
    ioctl(i2c_fd, I2C_SLAVE, SLAVE_ADDRESS);
    reg_addr[0] = REG_ADDRESS & 0x00FF;
    if((write(i2c_fd, reg_addr, 1)) < 0){
        printf("REG_ADDRESS Write Fail\r\n");
        return -1;
    }
